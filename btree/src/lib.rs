...
trait BTreeProxy<V: PartialOrd, I: BTreeIdx> {
    fn root(&self) -> I;
...
impl<V: PartialOrd + Debug, I: BTreeIdx + Debug, P: BTreeProxy<V, I>> BTree<V, I> for P {
fn max_depth_from(&self, node: I) -> usize {
    use core::cmp::max;
...
fn df_map_from<T, F: FnMut(I, V) -> Option<T>>(&self, idx: I, func: &mut F) -> Option<T> {
    ...
    loop {
        let mut left = self.get_left(node);
        ...
        if !done_left {
            ...
            let ret = func(node, self.value(node));
            if ret.is_some() {
                return ret;
            }
        ...
        if right.is_some() {
            ...
            if parent.is_some() {
                while parent.is_some() && (node != self.get_left(parent)) {
                    node = parent;
...
fn df_map<T, F: FnMut(I, V) -> Option<T>>(&self, func: &mut F) -> Option<T> {
    self.df_map_from(self.root(), func)
}
...
mod test {
    ...
    fn check<V: PartialOrd + Ord + Debug + Clone, I: BTreeIdx + PartialOrd + Ord + Debug, T: BTree<V, I> + Debug>(mut proxy: T, mut actual_values: Vec<(V, I)>) {
        actual_values.sort_by_key(|ab| ab.0.clone());
        let mut values = Vec::new();

        proxy.df_map(&mut |idx, value| {
